...
reads the configured intervals from FedEx Connector Settings and enqueues
...
def check_and_enqueue():
    if not frappe.db.exists("DocType", "FedEx Sync Log"):
        return
    ...
    _maybe_enqueue(
        interval_setting=settings.fedex_pull_sync_interval or "Disabled",
        sync_type="pull",
        enqueue_fn="alaiy_os_connector_fedex.fedex.sync.run_pull_sync",
    )
    _maybe_enqueue(
        interval_setting=settings.fedex_push_sync_interval or "Disabled",
        sync_type="push",
        enqueue_fn="alaiy_os_connector_fedex.fedex.sync.run_push_sync",
    )
